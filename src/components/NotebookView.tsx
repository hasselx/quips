        ...
        <DialogContent className="inset-x-0 bottom-0 left-0 top-auto flex h-[100dvh] max-h-[100dvh] w-full max-w-none translate-x-0 translate-y-0 flex-col gap-0 overflow-hidden rounded-none border-x-0 border-b-0 p-0 sm:left-1/2 sm:top-1/2 sm:h-auto sm:max-h-[90dvh] sm:max-w-md sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-xl sm:border">
          <DialogHeader className="shrink-0 border-b border-border px-4 py-3 text-left">
            <div className="flex items-center justify-between gap-3 pr-8">
              ...
              <Button type="button" variant="outline" size="sm" className="h-8 gap-1.5" onClick={handleEditParsedExpense} disabled={receiptBusy}>
          ...
          {parsedExpense && (
            <>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3 rounded-lg border border-border bg-card p-3">
                  {[
                    ...
                    <div key={label} className="min-w-0">
                      ...
                      <span className="mt-0.5 block break-words text-sm font-medium text-foreground">{value}</span>
                    </div>
                  ))}
                </div>
                        ...
                        <tbody>
                          {parsedItems.map((it, i) => (
                            <tr key={i} className="border-t border-border/60 align-top">
                              <td className="break-words px-3 py-2.5 leading-4 text-foreground">{it.name}</td>
                              <td className="px-1 py-2 text-center text-muted-foreground tabular-nums">{it.qty}</td>
                              <td className="whitespace-nowrap px-2 py-2 text-right font-medium tabular-nums text-foreground sm:px-3">{notebookCurrency.symbol}{it.price}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="grid shrink-0 grid-cols-2 gap-3 border-t border-border bg-background px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:px-4 sm:pb-4">
                ...
                <Button type="button" className="h-12 gap-1.5 rounded-lg" onClick={handleAddParsedExpense} disabled={receiptBusy}>
                  {receiptBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
                  {receiptStatus === "adding" ? "Submitting…" : "Submit"}
                </Button>
              </div>
            </>
          )}
